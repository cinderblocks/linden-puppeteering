...
struct LLParcelInterface
{
	virtual ~LLParcelInterface() = 0;
	virtual const std::string& getName() const = 0;
	virtual const LLUUID& getID() const = 0;
	virtual const LLUUID& getOwnerID() const = 0;
	virtual bool canModify(const LLUUID& agent) const = 0;
};

class LLParcel : public LLParcelInterface
{
public:
...
//	BOOL	exportStream(std::ostream& output_stream);
	...
	virtual bool canModify(const LLUUID& agent) const;
