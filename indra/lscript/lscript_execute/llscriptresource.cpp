...
{
	if ((mUsed + amount <= mTotal) || !isLimitEnforced())
	{
		mUsed += amount;
		return true;
	}
...
bool LLScriptResource::isLimitEnforced() const
{
	return mIsEnforced;
}

void LLScriptResource::isLimitEnforced(bool is_enforced)
{
	mIsEnforced = is_enforced;
}
