...
enum LLScriptResourceType
{
	SCRIPT_RESOURCE_URL,
	SCRIPT_RESOURCE_MEMORY,
...
public:
	...
	// true if the resource is actively enforced (i.e. over budget requests are denied)
	bool isLimitEnforced() const;

	// set whether this resource is actively enforced
	// Should be a const and set from ctor, however we initialize arrays of
	// resource
	void isLimitEnforced(bool is_enforced);

private:
	S32 mTotal; // How many resources have been set aside
	S32 mUsed; // How many resources are currently in use
	bool mIsEnforced; // Controls whether this resource is actively policed or simply observed
};
