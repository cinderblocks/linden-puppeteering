	...
	if (mUpstreamHost.isOk())
	{
		if (mVFS->getExists(asset_id, asset_type))
		{
			...
			else
			{
				...
				buffer[0] = 0;
				asset_size = 0;
				// setting the size to zero will trigger the simulator to ask 
				// for the asset via the UDP Xfer system, so we record that
				// we expect to eventually transfer this file 
				gXferManager->expectVFileForTransfer(asset_id.asString());
			}
