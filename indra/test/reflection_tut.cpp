...
namespace tut
{
  ...
  {
  public:
	TestReflectionData() : mInt(42), mString("foo"), mNullPtr(NULL), mPtr(new TestAggregatedData()), mRef(*(new TestAggregatedData)), mBool(true) {;}
	virtual ~TestReflectionData() {delete mPtr;}
	virtual const LLMetaClass& getMetaClass() const;
	
	static U32 getPropertyCount() {return 6;}
...
void LLMetaClassT<tut::TestReflectionData>::reflectProperties(LLMetaClass& meta_class)
{
	...
	reflectProperty(meta_class, "mBool", &tut::TestReflectionData::mBool);
}
...
namespace tut
{
  ...
  void TestReflectionObject::test<1>()
  {
	...
	ensure_not_equals(meta_class.findProperty("mBool"), null);
  }
  ...
  template<> template<>
  void TestReflectionObject::test<12>()
  {
	// Check bool property has correct value.	
    const LLMetaClass& meta_class = LLMetaClassT<TestReflectionData>::instance();
	ensure_equals(meta_class.findProperty("mBool")->getLLSD(this).asBoolean(), true);
  }
