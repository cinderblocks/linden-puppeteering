...
namespace tut
{
	...
	void v4color_object::test<4>()
	{
		...
		LLColor4 llcolor4;
		llcolor4.set(r,g,b);
		ensure("1:set:Fail to set the values ", ((r == llcolor4.mV[VX]) && (g == llcolor4.mV[VY]) && (b == llcolor4.mV[VZ])&& (1.f == llcolor4.mV[VW])));

		llcolor4.set(r,g,b,a);
		ensure("2:set:Fail to set the values ", ((r == llcolor4.mV[VX]) && (g == llcolor4.mV[VY]) && (b == llcolor4.mV[VZ])&& (a == llcolor4.mV[VW])));
		...
		ensure("4:set:Fail to set the values ", ((llcolor3.mV[VX] == llcolor4a.mV[VX]) && (llcolor3.mV[VY] == llcolor4a.mV[VY]) && (llcolor3.mV[VZ] == llcolor4a.mV[VZ])));

		F32 val = -.33f;
		llcolor4a.set(llcolor3,val);
		ensure("4:set:Fail to set the values ", ((llcolor3.mV[VX] == llcolor4a.mV[VX]) && (llcolor3.mV[VY] == llcolor4a.mV[VY]) && (llcolor3.mV[VZ] == llcolor4a.mV[VZ]) && (val == llcolor4a.mV[VW])));

		const F32 vec[4] = {.112f ,23.2f, -4.2f, -.0001f};
		LLColor4 llcolor4c;
		llcolor4c.set(vec);
		ensure("5:set:Fail to initialize ", ((vec[0] == llcolor4c.mV[VX]) && (vec[1] == llcolor4c.mV[VY]) && (vec[2] == llcolor4c.mV[VZ])&& (vec[3] == llcolor4c.mV[VW])));		
		...
		const F32 SCALE = 1.f/255.f;
		F32 r2 = r1*SCALE, g2 = g1* SCALE, b2 = b1* SCALE;
		ensure("6:set:Fail to initialize ", ((r2 == llcolor4.mV[VX]) && (g2 == llcolor4.mV[VY]) && (b2 == llcolor4.mV[VZ])));
	}
	...
	void v4color_object::test<6>()
	{
		...
		ensure("length:Fail ", is_approx_equal(llcolor4.length(), F32(sqrt(r*r + g*g + b*b))));
	}
